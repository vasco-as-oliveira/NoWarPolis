...
public class City implements Serializable{
  ...
  private void readNodes(){
    try{
      ...
      for (i=0;reader.hasNextLine();i++){
        String[] data = reader.nextLine().split(",");
        Node n1 = new Node(i, Long.parseLong(data[0]),Double.parseDouble(data[1]), Double.parseDouble(data[2]));
  ...
  public void addEtiqueta_Node(PoI poi){
    if(!(poi.location.id<this.cityMap.weightedTime.V())){
      throw new RuntimeException("ERROR: VERTICES OVERLOAD");
    }
    if((this.getEtiquetas().containsKey(poi.name) && getEtiquetas().get(poi.name).contains(poi.location.idPos))){
      throw new RuntimeException("ERROR: POINT OF INTEREST ALREADY EXISTS");
    }
  ...
  //  getcityMapDistance().addEdge(way.getFrom().id, way.getTo().id);
    ...
    this.cityMap.weightedTime.addEdge(new DirectedEdge(way.getFrom().id, way.getTo().id, way.time()));
  ...
  public ArrayList<User> usersThatVisitedPoIBetweenTimes(PoI poi, int t1, int t2){
    ...
    try {
      FileWriter writer = new FileWriter("src/Files/searches/UsersThatVisitedPoIBetweenTimes"+t1+"_"+t2+".txt");
  ...
  public ArrayList<PoI> PoIsNOTvisitedBetweenTimes(int start, int end){
    ...
    try {
      FileWriter writer = new FileWriter("src/Files/searches/PoIsNotVisitedBetweenTimes"+start+"_"+end+".txt");
  ...
  public ArrayList<User> TopFiveUsers_VisitedBiggerNumberOfPoIsBetweenTimes(int start, int end){
    ...
    try {
      FileWriter writer = new FileWriter("src/Files/searches/Top_5_Users_with_more_visits"+start+"_"+end+".txt");
  ...
  private void writeBin(){
    ...
    try {
      File file = new File("src/Files/dataset1_usersbin.bin");
      ...
      /*for(User user : this.users.values()){
        ...
        oos.writeObject(user.getBirth());
        oos.writeObject(user.getId());
        oos.writeObject(user.getVisitedNodes());
        writeBinBSTPoI(oos, user.getVisitedPoIs());
        oos.writeObject(user.getVisits());
       // writeBinNode(oos, user.getCurrentLocation());
        oos.writeObject(user.getVeiculos());
        oos.writeObject(user.getNumOfVisits());
      }*/
  ...
  public void writeBinGraph(ObjectOutputStream oos, DoubleWeightedGraph graph) throws IOException {
    ...
    oos.writeObject(graph.weightedTime.V());
    oos.writeObject(graph.weightedTime.E());

    oos.writeObject(graph.weightedDistance.V());
    oos.writeObject(graph.weightedDistance.E());

    for(DirectedEdge directedEdge : graph.weightedDistance.edges()){
      oos.writeObject(directedEdge.from());
      oos.writeObject(directedEdge.to());
      oos.writeObject(directedEdge.weight());
    }
    for(DirectedEdge directedEdge : graph.weightedTime.edges()){
      oos.writeObject(directedEdge.from());
      oos.writeObject(directedEdge.to());
      oos.writeObject(directedEdge.weight());
    }

    oos.writeObject(graph.ways);

    for(Integer keys : graph.times){
  ...
  private void writePoi(){
    try {
      FileWriter write = new FileWriter("src/Files/dataset1_PoI.txt");
  ...
  private void writeUser(){
    ...
    try {
      FileWriter write = new FileWriter("src/Files/dataset1_users.txt");
